...
(function(){
  ...
  // Kleine UI-Ergänzung im Matchscreen: Engine-Hinweis sichtbar machen, ohne den vorhandenen Screen umzubauen.
  const previousMatchScreen = typeof matchScreen === 'function' ? matchScreen : null;
  matchScreen = function() {
    const html = previousMatchScreen ? previousMatchScreen() : '<section class="panel"><h2>Spiel</h2></section>';
    const m = state.activeMatch;
    if (!m || !m.engineSummary || html.includes('Stochastische v136-Engine')) return html;
    const info = `<div class="infoBox"><b>Stochastische v136-Engine:</b> Angriff/Defensive werden aus Stärke, Form, Frische, Moral, Taktik, Heimvorteil, Momentum, Platzverweisen und Konditionsabbau berechnet. Intern wurde das komplette Spiel bereits als Ereignisobjekt vorbereitet.</div>`;
    return html.replace('<button class="primary full" onclick="simulateFirstHalf()">', `${info}<button class="primary full" onclick="simulateFirstHalf()">`);
  };
